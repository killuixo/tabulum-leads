...
// ==========================================
// ABA 1: VISUALIZAÇÃO PRINCIPAL (AGRUPADA, ALFABÉTICA E COM FILTROS)
// ==========================================
function LeadsView({ leads }) {
  ...
  // Estado dos filtros (Múltiplas seleções)
  const [filters, setFilters] = useState({ cidade: [], bairro: [], origem: [] });

  const itemsPerPage = 24;

  // 1. Agrupar por NOME (Tratar Repetidos) e Ordenar Alfabeticamente
  const { groupedLeads, filterOptions } = useMemo(() => {
    const map = new Map();
    const opts = { cidade: {}, bairro: {}, origem: {} };

    leads.forEach((l) => {
      // Padroniza o nome para agrupar corretamente
      ...
      if (!map.has(key)) {
        ...
        // Se alguma das repetições tiver uma cidade/bairro melhor e a primeira não tinha, poderíamos atualizar, 
        // mas vamos manter os dados da primeira ocorrência para consistência do agrupamento base.
      }
    });

    const groupedArray = Array.from(map.values()).sort((a, b) => a.nome.localeCompare(b.nome));

    // Popula as contagens dos filtros BASEADO NO AGRUPAMENTO ÚNICO
  ...
  // 2. Aplicar Pesquisa e Filtros
  const filteredLeads = useMemo(() => {
    return groupedLeads.filter((lead) => {
      // Filtro Texto Livre (Search)
...
function DashboardView({ leads }) {
  ...
  const stats = useMemo(() => {
    ...
    // O Dashboard usa a mesma lógica de únicos: nomes únicos.
    const uniqueNames = new Set(leads.map(l => (l.nome || 'Sem Nome').trim().toLowerCase()));
    const unique = uniqueNames.size;
    ...
    // Para estatísticas de ranking, utilizamos a primeira ocorrência do nome
    const processedMap = new Map();
    leads.forEach(l => {
      const key = (l.nome || 'Sem Nome').trim().toLowerCase();
      if (!processedMap.has(key)) processedMap.set(key, l);
    });

    processedMap.forEach(l => {
      const rawCid = (l.cidade || '').trim();
      const cid = rawCid || 'Não Informado';
      const isFln = rawCid.toLowerCase().includes('florian') || rawCid.toLowerCase().includes('floripa');
      const bairro = (l.bairroReplan || l.bairroRevisado || '').trim() || 'Não Informado';
      
      // Contagem Cidades (Omite Florianópolis e Não Informado do Top)
      ...
      // Contagem Bairros Apenas Florianópolis
      if (isFln) {
      ...
      const orig = (l.origem || '').trim() || 'Não Informado';
      origemCount[orig] = (origemCount[orig] || 0) + 1;
    });

    // Ordenações (Pegamos mais itens para o Mapa de Calor ficar denso)
    ...
    return { 
      ...
      heatCidades: topCidades.slice(0, 20), // Mapa de calor
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Gráfico Nativo: Top Cidades (Sem Floripa) */}
        ...
        {/* Gráfico Nativo: Top Bairros (Só Floripa) */}
        <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          {stats.topOrigens.map((origem, index) => {
            ...
            return (
              <div 
                ...
                className="h-full border-r-2 border-black last:border-r-0 hover:opacity-80 transition-opacity"
      ...
      {/* NOVO: Mapas Literais (Geográficos) */}
      ...
      <div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
        <div className="flex flex-col sm:flex-row border-b-4 border-black">
          <div className="p-4 sm:p-6 bg-yellow-400 border-b-4 sm:border-b-0 sm:border-r-4 border-black flex items-center justify-center gap-3 w-full sm:w-1/3 shrink-0">
            <MapIcon className="w-8 h-8" />
            <h3 className="text-xl font-black uppercase">Visão Árvore</h3>
          </div>
          <div className="flex w-full">
            <button 
              onClick={() => setActiveVisualMap('SC')}
              className={`flex-1 py-4 font-black uppercase border-r-4 border-black transition-colors ${activeVisualMap === 'SC' ? 'bg-black text-white' : 'bg-white hover:bg-gray-100'}`}
            >
              Santa Catarina (Cidades)
            </button>
            <button 
              onClick={() => setActiveVisualMap('FLN')}
              className={`flex-1 py-4 font-black uppercase transition-colors ${activeVisualMap === 'FLN' ? 'bg-black text-white' : 'bg-white hover:bg-gray-100'}`}
            >
              Florianópolis (Bairros)
            </button>
          </div>
        </div>
        ...
        <div className="p-4 md:p-6 bg-gray-50 flex flex-col items-center">
          ...
          <div className="flex flex-wrap gap-1 justify-center w-full max-w-5xl">
            {(activeVisualMap === 'SC' ? stats.heatCidades : stats.heatBairros).map((item, index) => {
              // Estilização Mondrian Dinâmica e densa
              let bg = 'bg-white';
              let text = 'text-black';
              ...
              // Tamanho dinâmico do bloco (Mais blocos e mais próximos)
            ...
            {(activeVisualMap === 'SC' ? stats.heatCidades : stats.heatBairros).length === 0 && <p className="font-bold p-8">Sem dados geográficos para este filtro.</p>}
          </div>

          {/* Dados Não Informados (Discretos) */}
          <div className="mt-4 w-full max-w-5xl flex justify-end">
             <div className="text-[11px] font-bold text-gray-500 bg-gray-200 border-2 border-gray-300 px-3 py-1 uppercase tracking-wider">
               Leads s/ {activeVisualMap === 'SC' ? 'Cidade' : 'Bairro Floripa'}: {activeVisualMap === 'SC' ? stats.naoInformadoCidade : stats.naoInformadoBairroFln}
             </div>
          </div>
...
// --- DICIONÁRIO DE COORDENADAS PARA MAPAS GEOGRÁFICOS ---
// Escala 0 a 100.
const SC_COORDS = {
  'florianópolis': [88, 60], 'são josé': [82, 59], 'palhoça': [80, 66], 'biguaçu': [78, 53], 
  'joinville': [75, 20], 'blumenau': [70, 35], 'itajaí': [85, 30], 'balneário camboriú': [86, 36], 
  'criciúma': [65, 85], 'tubarão': [75, 80], 'garopaba': [85, 72], 'lages': [50, 65], 
  'chapecó': [20, 45], 'joaçaba': [35, 45], 'concórdia': [25, 50], 'rio do sul': [60, 45], 
  'brusque': [78, 40], 'jaraguá do sul': [72, 22], 'mafra': [60, 15], 'caçador': [40, 30], 
  'são miguel do oeste': [5, 45], 'laguna': [80, 78], 'imbituba': [83, 75]
};
...
function LiteralMap({ type, data }) {
  ...
  // Caminhos de vetor simplificados e geométricos
  const scPath = "M 5 45 L 20 20 L 50 15 L 75 20 L 95 35 L 95 70 L 70 95 L 45 90 L 15 65 Z";
  ...
  return (
    <div className="relative w-full max-w-2xl aspect-square sm:aspect-video bg-white border-[6px] border-black p-4">
       <svg viewBox="0 0 100 100" className="w-full h-full" preserveAspectRatio="xMidYMid meet">
         {/* Renderiza as formas base do mapa */}
         {type === 'SC' ? (
         ...
         {/* Renderiza os blocos Mondrian de dados por cima */}
         {data.map(item => {
            const coords = dict[item.name.toLowerCase()];
            if (!coords) return null; // Ignora se não estiver mapeado no dicionário visual
            
            // Calcula o tamanho do bloco (quadrado) entre 2 e 8 (escala SVG)
       ...
       {/* Tooltip personalizado flutuante */}
       {hoveredNode && (
         <div 
