...
function LeadsView({ filteredLeads, viewMode }) {
  ...
  // Reseta a página se a lista filtrada mudar
...
function DashboardView({ filteredLeads }) {
  const [activeVisualMap, setActiveVisualMap] = useState('SC'); // 'SC' ou 'FLN' para os Mapas Literais
  ...
  const stats = useMemo(() => {
    ...
    return { 
      ...
      mapCidades: topCidades, // Para o mapa geográfico
      topBairrosFln: topBairrosFln.slice(0, 6), 
      heatBairros: topBairrosFln.slice(0, 20),
      mapBairros: topBairrosFln, // Para o mapa geográfico
